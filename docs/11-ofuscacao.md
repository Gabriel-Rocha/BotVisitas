...
2. **Não expor proxy/VPN** — em especial, não vazar o IP real (WebRTC) nem deixar
   headers/timezone incoerentes com o tráfego.
...
| Patches de página | `applyPageStealth` | `navigator.webdriver`, bloqueio WebRTC, `chrome.runtime`, languages |
...
- Não remover o bloqueio de WebRTC sem decisão explícita documentada.
...
WebRTC se não houver patch — por isso o bloqueio é obrigatório quando o objetivo
é ofuscar o caminho de rede. Ver [`09-proxies-webshare.md`](./09-proxies-webshare.md).
